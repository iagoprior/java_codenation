...
public class Main {
	...
	public int q1() {
        ...
        nationalityPlayers = players.stream().collect(Collectors.groupingBy(x -> x[columns.indexOf("nationality")])).size();
	...
	public int q2() {
        ...
        int club;
        club = players.stream().filter(x -> !x[columns.indexOf("club")].isEmpty()).collect(Collectors.groupingBy(x -> x[columns.indexOf("club")])).size();
        return club;
	...
	public List<String> q4() {
        ...
        top10 = players.stream()
                .filter(j -> !j[columns.indexOf("eur_release_clause")].isEmpty())
                .sorted(Comparator.comparing(j -> Double.parseDouble(j[columns.indexOf("eur_release_clause")])))
                .map(x -> x[columns.indexOf("full_name")])
	...
	public List<String> q5() {
        ...
        top10old = players.stream()
                .filter(j -> !j[columns.indexOf("birth_date")].isEmpty())
	...
	public Map<Integer, Integer> q6() {
        ...
        return players.stream().collect(Collectors.groupingBy(j -> Integer.parseInt(j[columns.indexOf("age")]), Collectors.reducing(0, e -> 1, Integer::sum)));
	}
}
